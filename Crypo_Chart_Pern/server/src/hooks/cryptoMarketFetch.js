  ...
  if (data) {
    try {
      ...
      data.forEach((coin, index) => {
        ...
        values += `(${rank},'${coin.id}','${coin.symbol}', '${coin.name}', '${coin.image}', ${coin.current_price}, ${coin.market_cap}, ${coin.market_cap_rank}, ${coin.fully_diluted_valuation}, ${coin.total_volume}, ${volume24hr}, ${coin.high_24h}, ${coin.low_24h}, ${coin.price_change_24h}, ${coin.price_change_percentage_24h}, ${coin.market_cap_change_24h}, ${coin.market_cap_change_percentage_24h}, ${coin.circulating_supply}, ${coin.total_supply}, ${coin.max_supply}, ${current}),`;
      });

      //   Remove last coma
      values = values.slice(0, -1);
      await db.query(
        `INSERT INTO crypto_market (rank ,crypto_id, symbol, name, image, current_price, market_cap, market_cap_rank, fully_diluted_valuation, total_volume, volume_24hr, high_24h, low_24h, price_change_24h, price_change_percentage_24h, market_cap_change_24h, market_cap_change_percentage_24h, circulating_supply, total_supply, max_supply, unix) VALUES ${values} ON CONFLICT (crypto_id) DO UPDATE SET rank = EXCLUDED.rank, symbol = EXCLUDED.symbol, name = EXCLUDED.name, image = EXCLUDED.image, current_price = EXCLUDED.current_price, market_cap = EXCLUDED.market_cap, market_cap_rank = EXCLUDED.market_cap_rank, fully_diluted_valuation = EXCLUDED.fully_diluted_valuation, total_volume = EXCLUDED.total_volume, volume_24hr = EXCLUDED.volume_24hr, high_24h = EXCLUDED.high_24h, low_24h = EXCLUDED.low_24h, price_change_24h = EXCLUDED.price_change_24h, price_change_percentage_24h = EXCLUDED.price_change_percentage_24h, market_cap_change_24h = EXCLUDED.market_cap_change_24h, market_cap_change_percentage_24h = EXCLUDED.market_cap_change_percentage_24h, circulating_supply = EXCLUDED.circulating_supply, total_supply = EXCLUDED.total_supply, max_supply = EXCLUDED.max_supply, unix = EXCLUDED.unix;`
      );
    } catch (err) {
